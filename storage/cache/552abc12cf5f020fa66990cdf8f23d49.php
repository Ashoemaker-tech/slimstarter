<nav x-data="{ mobileMenu: false }" class="bg-gray-800">
    <div class="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
        <div class="relative flex h-16 items-center justify-between">
          ...
          -->
                    <svg :class="mobileMenu ? 'hidden' : 'block'" class=" h-6 w-6" fill="none" viewBox="0 0 24 24"
          ...
          -->
                    <svg :class="mobileMenu ? 'block' : 'hidden'" x-transition class=" h-6 w-6" fill="none"
            ...
            <div class="flex flex-1 items-center justify-center sm:items-stretch sm:justify-start">
                <div class="flex flex-shrink-0 items-center">
                    <a href="/">
                        <span class="text-white font-bold text-lg">LaraSlim</span>
                    </a>
                </div>
            ...
            <div class="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
                <?php if(auth()): ?>
                    <span class="text-gray-100 font-sm hidden sm:block">Welcome,
                        <?php echo e(auth()->username ?? 'Guest'); ?></span>
                    <!-- Profile dropdown -->
                    <div x-data="{ dropdownOpen: false }" class="relative ml-3">
                        ...
                        <div>
                            <button type="button" @click="dropdownOpen = !dropdownOpen"
                                ...
                                <img class="h-8 w-8 rounded-full" src="<?php echo e(asset('images/user.png')); ?>" alt="">
                            </button>
                        </div>

                        <div x-show="dropdownOpen" x-transition x-cloak @click.outside="dropdownOpen = false"
                            class="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
                            role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button"
                            tabindex="-1">
                            <!-- Active: "bg-gray-100", Not Active: "" -->
                            <a href="/account" class="block px-4 py-2 text-sm text-gray-700" role="menuitem"
...
<?php /**PATH C:\laragon\www\slimstarter\resources\views/components/navbar.blade.php ENDPATH**/ ?>